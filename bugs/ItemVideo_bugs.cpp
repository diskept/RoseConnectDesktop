...
#include "bugs/ItemImage_Video_bugs.h"
...
namespace bugs {
     ...
     * @param parent
     */
    ItemVideo_bugs::ItemVideo_bugs(int index, int section, ImageSizeMode imageSizeMode, QWidget *parent)
     ...
     */
    void ItemVideo_bugs::setUIControl(){

        int base_height = 0;
        int img_width = 0;

        this->itemImage = new ItemImage_Video_bugs(this->get_imageWidth(this->m_imageSizeMode), this->get_imageHeight(this->m_imageSizeMode));
        connect(this->itemImage, &ItemImage_Video_bugs::signal_clickedBtn, this, &ItemVideo_bugs::slot_itemImage_clickedBtn);
     ...
     */
    void ItemVideo_bugs::setData(const bugs::VideoItemData &p_data_video){
        this->data_video = p_data_video;
    }


    /**
     * @brief ItemVideo_bugs::get_fixedHeight
     * @return
     */
    int ItemVideo_bugs::get_fixedHeight(){
     ...
     * @param event
     */
    void ItemVideo_bugs::paintEvent(QPaintEvent *event){
        ...
        if(this->flagInitDraw == false){
            ...
            this->itemImage->setImage(this->data_video.image);
            qobject_cast<ItemImage_Video_bugs*>(this->itemImage)->setDuration(this->data_video.len);

            int all_width = this->itemImage->sizeHint().width();

            QString title = this->data_video.mv_title;
