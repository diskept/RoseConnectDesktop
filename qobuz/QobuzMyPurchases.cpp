...
namespace qobuz {
     ...
     * @param parent
     */
    QobuzMyPurchases::QobuzMyPurchases(QWidget *parent) : AbstractQobuzSubWidget(MainUIType::VerticalScroll_filter, parent) {
     ...
     * @brief 소멸자.
     */
    QobuzMyPurchases::~QobuzMyPurchases(){
     ...
     * @param jsonObj
     */
    void QobuzMyPurchases::setJsonObject_forData(const QJsonObject &jsonObj){
        ...
        if(pageCode != this->page){
            ...
            this->page = PAGECODE_Q_MY_COLLECTION_PURCHASES;

            if(this->flag_album[1] == true || this->flag_track[1] == true){
            ...
            connect(proc_a, &ProcCommon::completeReq_list_myPurchasesAlbums, this, &QobuzMyPurchases::slot_applyResult_myFavoriteAlbums);
            proc_a->request_qobuz_getList_myPurchasesAlbums(sortOpt, orderDirection, 15, 0);
     ...
     * HTTP API 호출한다.
     */
    void QobuzMyPurchases::setActivePage(){
        ...
        if(this->flagNeedReload){
            ...
            for(int i = 0; i < 15; i++){
                this->search_album[i] = new qobuz::ItemAlbum_qobuz(i, SECTION_FOR_MORE_POPUP___album, tidal::AbstractItem::ImageSizeMode::Square_200x200);
                connect(this->search_album[i], &qobuz::ItemAlbum_qobuz::signal_clicked, this, &QobuzMyPurchases::slot_clickedItemAlbum);
            }

            for (int i = 0; i < 5; i++) {
                this->search_track[i] = new PlaylistTrackDetailInfo_RHV;
                connect(this->search_track[i], &PlaylistTrackDetailInfo_RHV::clicked, this, &QobuzMyPurchases::slot_clickedItemTrack_inList);
                this->search_track[i]->setProperty("index", i);
                this->search_track[i]->setObjectName("tracks");
            }
        }
    }
     ...
     */
    void QobuzMyPurchases::setUIControl_basic(){

        this->addUIControl_mainTitle(tr("My Purchases"));
    }


    void QobuzMyPurchases::setUIControl_appendWidget(){

        if(true && flag_album[0] == true && flag_track[0] == true){

            if(flag_album[1] == true){
                this->widget_album = new QWidget();
                this->widget_album = this->setUIControl_subTitle_withSideBtn(QString("Albums (%1)").arg(this->list_album->at(0).album_total_cnt), "View All", BTN_IDX_SUBTITLE_album, this->vBox_album);
                ...
                this->widget_album = new QWidget();
                this->widget_album = this->setUIControl_subTitle_withSideBtn(QString("Albums (0)"), "", BTN_IDX_SUBTITLE_album, this->vBox_album);
            ...
            if(flag_track[1] == true){
                ...
                connect(proc_fav_track, &roseHome::ProcCommon::completeReq_rating_track, this, &QobuzMyPurchases::slot_applyResult_getRating_track);
    ...
    QWidget* QobuzMyPurchases::setUIControl_subTitle_withSideBtn(const QString subTitle, const QString btnText, const int btnId, QLayout *p_layout){
    ...
    QHBoxLayout* QobuzMyPurchases::setUIControl_hBoxLayout_forAlbum(QLayout *p_layout){
        //----------------------------------------------------------------------------------------------------  BODY : START
     ...
     * @param btnId
     */
    void QobuzMyPurchases::slot_clickBtn_subTitle_viewAll(){

        int btnId = sender()->property("idx").toInt();

        if(btnId == BTN_IDX_SUBTITLE_album){
            QJsonObject tmp_data;
            tmp_data.insert(KEY_PAGE_CODE, PAGECODE_Q_MY_COLLECTION_PURCHASES_ALBUM_LIST);
            emit this->signal_clickedMovePage(tmp_data);        // 부모에게 페이지 변경하라고 시그널 보냄
       }
       else if(btnId == BTN_IDX_SUBTITLE_track){
            QJsonObject tmp_data;
            tmp_data.insert(KEY_PAGE_CODE, PAGECODE_Q_MY_COLLECTION_PURCHASES_TRACK_LIST);
            emit this->signal_clickedMovePage(tmp_data);        // 부모에게 페이지 변경하라고 시그널 보냄
       }
    }
     ...
     * @param flag_lastPage
     */
    void QobuzMyPurchases::slot_applyResult_myFavoriteAlbums(const QList<qobuz::AlbumItemData> &list_data, const QJsonArray& jsonArr_data, const bool flag_lastPage){
     ...
     * @param jsonObj_track
     */
    void QobuzMyPurchases::slot_applyResult_myFavoriteTracks(const QList<qobuz::TrackItemData> &list_data, const QJsonArray& jsonArr_dataToPlay, const bool flag_lastPage){
    ...
    void QobuzMyPurchases::slot_qobuz_completeReq_listAll_myFavoritesIds(const QJsonObject& p_jsonObj){
        ...
        if(p_jsonObj.contains("flagOk") && ProcJsonEasy::get_flagOk(p_jsonObj)){
            ...
            if(this->flag_send_track == true){
                if((status == true && this->flag_track_fav == false) || (status == false && this->flag_track_fav == true)){
                    ...
                    connect(proc, &qobuz::ProcCommon::completeReq_listAll_myFavoritesIds, this, &QobuzMyPurchases::slot_qobuz_completeReq_listAll_myFavoritesIds);
    ...
    void QobuzMyPurchases::slot_applyResult_checkRating_track(const QJsonObject &jsonObj){
        ...
        if(id <= 0){
            ...
            // request HTTP API - get favorite for Rose Server
            roseHome::ProcCommon *proc_fav_track = new roseHome::ProcCommon(this);
            connect(proc_fav_track, &roseHome::ProcCommon::completeCheck_rating_track, this, &QobuzMyPurchases::slot_applyResult_addRating_track);
    ...
    void QobuzMyPurchases::slot_applyResult_addRating_track(const QJsonObject &jsonObj){
    ...
    void QobuzMyPurchases::slot_applyResult_getRating_track(const QJsonArray &contents){
     ...
     * @param clickMode
     */
    void QobuzMyPurchases::slot_clickedItemAlbum(const tidal::AbstractItem::ClickMode clickMode){
     ...
     * @param clickMode
     */
    void QobuzMyPurchases::slot_clickedItemTrack_inList(const int idx, const PlaylistTrackDetailInfo_RHV::ClickMode clickMode){
        ...
        if(clickMode == PlaylistTrackDetailInfo_RHV::ClickMode::FavBtn){
            ...
            if(this->flag_check_track == false){
                ...
                connect(proc_favCheck_track, &roseHome::ProcCommon::completeCheck_rating_track, this, &QobuzMyPurchases::slot_applyResult_checkRating_track);
     ...
     * @param section
     */
    void QobuzMyPurchases::slot_optMorePopup_menuClicked(const OptMorePopup::ClickMode clickMode, const int index, const int section){
