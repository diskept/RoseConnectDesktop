...
void FrameMusicComposer::setUIControl(){
    ...
    this->lb_composer = new QLabel(label_base);
    this->lb_composer->setWordWrap(true);
    this->lb_composer->setTextInteractionFlags(Qt::TextSelectableByMouse);//cheon210714-mousecopy
    this->lb_composer->setStyleSheet("font-size:16px;font-weight: normal;font-style: normal;line-height: 1.25;text-align: left; color:#FFFFFF;");
    this->lb_composer->setGeometry(0, 0, img_width, this->LABEL_HEIGHT);
...
void FrameMusicComposer::setPopupData(){
    ...
    if(dataPopupList->size()==0){
        ...
        if(err.type() == QSqlError::NoError){
            ...
            strQuery += " WHERE A.composer='"+data->getComposer() + "' ORDER BY A.bookmark ASC, A.track ASC, orderName ASC ";
            QVariantList dataDB;
...
void FrameMusicComposer::paintEvent(QPaintEvent *event){
    ...
    if(flagNeedDraw){
        ...
        this->lb_composer->setText(GSCommon::getTextCutFromLabelWidth(data->getComposer(), IMG_WIDTH, lb_composer->font()));
        if(this->lb_composer->text().contains("…")){
            this->lb_composer->setToolTip(data->getComposer());
            this->lb_composer->setToolTipDuration(2000);
        }
        this->lb_numSongs->setText(GSCommon::getTextCutFromLabelWidth(QString("%1 %2").arg(data->getNumber_of_tracks()).arg(QString(tr("Song"))), IMG_WIDTH, lb_numSongs->font()));
        if(this->lb_numSongs->text().contains("…")){
            this->lb_numSongs->setToolTip(QString("%1 %2").arg(data->getNumber_of_tracks()).arg(QString(tr("Song"))));
