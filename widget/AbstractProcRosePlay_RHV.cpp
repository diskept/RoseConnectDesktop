 ...
 * @param parent
 */
AbstractProcRosePlay_RHV::AbstractProcRosePlay_RHV(QWidget *parent) : QObject(parent) {
    this->parent = parent;
 ...
 * @return
 */
int AbstractProcRosePlay_RHV::get_rose_playType(OptMorePopup::ClickMode clickMode){
 ...
 * @return
 */
QJsonArray AbstractProcRosePlay_RHV::get_rearrangeJsonArray_toPlayData(const QJsonArray& jsonArr_toPlayAll, const int curr_index, OptMorePopup::ClickMode clickMode){
    if(clickMode == OptMorePopup::ClickMode::Play_RightNow_withReorder
        ...
        //jsonArr_output.append(jsonArr_toPlayAll.at(0).toObject());//cheon211005
        return jsonArr_output;
    }
    else if(clickMode == OptMorePopup::ClickMode::SubMenu_Play_FromHere
 ...
 * @return
 */
QJsonArray AbstractProcRosePlay_RHV::subRange_JsonArray(const QJsonArray &p_jsonArr, const int startIndex){
 ...
 * @return
 */
QJsonArray AbstractProcRosePlay_RHV::reorderJsonArray(const QJsonArray &p_jsonArr, const int startIndex){
    const int len_data = p_jsonArr.count();
    if(startIndex > 0 && startIndex < len_data){
