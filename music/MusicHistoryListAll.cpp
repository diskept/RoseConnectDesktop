...
namespace music {
    ...
    /**
     * @brief HistoryListAll::MusicHistoryListAll
     * @param parent
     */
    HistoryListAll::HistoryListAll(QWidget *parent) : AbstractRoseHomeSubWidget(MainUIType::VerticalScroll_roseviewAll, parent) {
     ...
     * @param jsonObj
     */
    void HistoryListAll::setJsonObject_forData(const QJsonObject &jsonObj){
     ...
     * HTTP API 호출한다.
     */
    void HistoryListAll::setActivePage(){
     ...
     * @brief 기본 UI 구성함
     */
    void HistoryListAll::setUIControl_history(){
     ...
     * @param list_data
     */
    void HistoryListAll::slot_applyResult_history(const QList<roseHome::HistoryItemData> &list_data, const QJsonArray &jsonArr_dataToPlay, const bool flag_lastPage){
        ...
        if(list_data.size() > 0){
            ...
            if(start_index == 0){
                ...
                for(int i = start_index; i < max_cnt; i++){
                    connect(this->listHistory_all[i], &roseHome::ItemHistory_rosehome::signal_clicked, this, &HistoryListAll::slot_clickedItemAlbum);
     ...
     * @param clickMode
     */
    void HistoryListAll::slot_clickedItemAlbum(const tidal::AbstractItem::ClickMode clickMode){

        int index = ((tidal::AbstractItem*)sender())->index();
        int section = ((tidal::AbstractItem*)sender())->section();

        if(section == SECTION_FOR_MORE_POPUP___history){
            // ClickMode 별로 처리
     ...
     * @param event
     */
    void HistoryListAll::resizeEvent(QResizeEvent *event){//c230223

        Q_UNUSED(event);

        if(this->width() - (this->history_widget_width+this->history_widget_margin)>= 0){
