...
namespace music {
    ...
    MusicList_Group::MusicList_Group(GroupMode p_mode, QWidget *parent) : roseHome::AbstractRoseHomeSubWidget(MainUIType::VerticalScroll_musicviewAll, parent)
    {
    ...
    MusicList_Group::MusicList_Group(QString p_strArg, GroupMode p_mode, QWidget *parent)
        :roseHome::AbstractRoseHomeSubWidget(MainUIType::VerticalScroll_musicviewAll, parent)
        , strArg_1(p_strArg)
    {
        this->groupMode = p_mode;
        setUIControl();
    ...
    MusicList_Group::~MusicList_Group(){
    ...
    void MusicList_Group::setDataFromDB(){
        ...
        this->jsonArr_Item = QJsonArray();

        if(groupMode==GroupMode::KIND){
            this->total_cnt = this->getDataFromDBforGenreTotalCount();
            //qDebug() << "[Debug] MusicList_Group::setDataFromDB " << this->total_cnt;
        }
        else if(groupMode==GroupMode::ARTIST){
            this->total_cnt = this->getDataFromDBforArtistTotalCount();
            //qDebug() << "[Debug] MusicList_Group::setDataFromDB " << this->total_cnt;
        }
        else if(groupMode==GroupMode::COMPOSER){
            this->total_cnt = this->getDataFromDBforComposerTotalCount();
            //qDebug() << "[Debug] MusicList_Group::setDataFromDB " << this->total_cnt;
        }
        else if(groupMode==GroupMode::VIEWALL_ADDEDALBUM){
            this->total_cnt = this->getDataFromDBforRecentlyAddTotalCount();
            //qDebug() << "[Debug] MusicList_Group::setDataFromDB " << this->total_cnt;
        }
    }
    ...
    void MusicList_Group::setUIControl(){
        ...
        this->flowLayout_items = this->get_addUIControl_flowLayout(0, 20);
    }


    int MusicList_Group::getDataFromDBforGenreTotalCount(){
    ...
    int MusicList_Group::getDataFromDBforArtistTotalCount(){
    ...
    int MusicList_Group::getDataFromDBforComposerTotalCount(){
    ...
    int MusicList_Group::getDataFromDBforRecentlyAddTotalCount(){
    ...
    void MusicList_Group::setDataGenreFromDB(){
        ...
        if(this->total_cnt > this->jsonArr_Item.count()){
            // next_offset
            if(this->jsonArr_Item.count() == 0){
                this->next_offset = 0;
            }

            int limit_count = (this->total_cnt - this->jsonArr_Item.count()) > 500 ? 500 : (this->total_cnt - this->jsonArr_Item.count());
            ...
            if(err.type() == QSqlError::NoError){
                ...
                */
                ...
                QString strQuery = "";
                strQuery = " SELECT aiTable.genre_id as _id , genres_info.name, genres_info.numalbums, genres_info.numsongs, UPPER(genres_info.name) as head ,album_art._data as album_art ";
                strQuery += " FROM (select genres_info._id as genre_id, max(audio._id) as audio_id, max(album_art.album_id) as album_id ,album_art._data ";
                ...
                //qDebug() << "[Debug] MusicList_Group::setDataGenreFromDB " << strQuery;
                ...
                if(data.size() > 0){
                    ...
                    int start_index = this->draw_cnt;

                    if(start_index == 0){
                        // widget draw change - by diskept j230317 start
                        int f_width = this->flowLayout_items->geometry().width();

                        if((this->width() - (80 + 63) - 10) > f_width){
                            f_width = this->width() - (80 + 63) - 10;
                        }
                        else if(this->flowLayout_items->geometry().width() <= 0){
                            f_width = this->width() - (80 + 63) - 10;
                        }

                        int f_wg_cnt = f_width / (200 + 17);
                        int f_mod = this->draw_cnt % f_wg_cnt;

                        if(f_mod == 0){
                            this->widget_cnt = f_wg_cnt * 30;
                        }
                        else{
                            this->widget_cnt = f_wg_cnt * 30 + (f_wg_cnt - f_mod);
                        }

                        //qDebug() << "[Debug] MusicList_Group::setDataGenreFromDB " << this->total_cnt << this->widget_cnt << this->draw_cnt;

                        int max_cnt = ((this->total_cnt - this->draw_cnt) > this->widget_cnt ) ? this->widget_cnt : (this->total_cnt - this->draw_cnt);
                        if(max_cnt > data.size()){
                            max_cnt = data.size();
                        }
                        this->draw_cnt += max_cnt;

                        //qDebug() << "[Debug] MusicList_Group::setDataGenreFromDB " << start_index << max_cnt;

                        for(int i = 0; i < data.size(); i++){
                            this->jsonArr_Item.append(data.at(i).toJsonObject());
                        }
                        ...
                        for(int i = start_index; i < max_cnt; i++){
                            ...
                            tmp_widget->setData(this->jsonArr_Item.at(i).toObject());
                            tmp_widget->setHover();

                            this->flowLayout_items->addWidget(tmp_widget);
                        }

                        // flowlayout spacing change - by diskept j230317 start
                        this->setFlowLayoutResize(this, this->flowLayout_items, 200, 17);
                        ...
                        // widget draw change - by diskept j230317 start
                        this->flag_item_draw = true;

                        int f_width = this->flowLayout_items->geometry().width();

                        if(this->flowLayout_items->geometry().width() <= 0){
                            f_width = this->width() - (80 + 63) - 10;
                        }
                        ...
                        //qDebug() << "[Debug] MusicList_Group::request_more_albumDraw " << f_width << f_wg_cnt << f_mod << this->widget_cnt;

                        start_index = this->draw_cnt;
                        int max_cnt = ((this->total_cnt - this->draw_cnt) > this->widget_cnt ) ? this->widget_cnt : (this->total_cnt - this->draw_cnt);
                        this->draw_cnt += max_cnt;

                        for(int i = start_index; i < this->draw_cnt; i++){
                            FrameMusicGenre *tmp_widget = new FrameMusicGenre;
                            tmp_widget->setData(this->jsonArr_Item.at(i).toObject());
                            tmp_widget->setHover();
            ...
            //qDebug() << "[Debug] MusicList_Group::request_more_albumDraw " << f_width << f_wg_cnt << f_mod << this->album_widget_cnt;
    ...
    void MusicList_Group::setDataArtistFromDB(){
        ...
        if(this->total_cnt > this->jsonArr_Item.count()){
            ...
            if(err.type() == QSqlError::NoError){
                ...
                strQuery += " (SELECT _data FROM album_art WHERE album_id=M.album_id) AS album_art";
                strQuery += " FROM artist_info AS AI INNER JOIN artists_albums_map AS M ON AI._id=M.artist_id ";
                strQuery += " GROUP BY AI._id ORDER BY AI.artist ";
                strQuery += QString(" LIMIT %1 ").arg(limit_count) + QString("OFFSET %1 ").arg(this->next_offset);

                this->next_offset += limit_count;

                //qDebug() << "[Debug] MusicList_Group::setDataArtistFromDB " << strQuery;
                ...
                if(data.size() > 0){
                    ...
                    if(start_index == 0){
                        ...
                        //qDebug() << "[Debug] MusicList_Group::setDataArtistFromDB " << this->total_cnt << this->widget_cnt << this->draw_cnt;
                        ...
                        //qDebug() << "[Debug] MusicList_Group::setDataArtistFromDB " << start_index << max_cnt << data.size();
                        ...
                        //qDebug() << "[Debug] MusicList_Group::setDataArtistFromDB " << f_width << f_wg_cnt << f_mod << this->widget_cnt;

                        start_index = this->draw_cnt;
                        int max_cnt = ((this->total_cnt - this->draw_cnt) > this->widget_cnt ) ? this->widget_cnt : (this->total_cnt - this->draw_cnt);
                        this->draw_cnt += max_cnt;

                        //qDebug() << "[Debug] MusicList_Group::setDataArtistFromDB " << start_index << this->draw_cnt << data.size();
            ...
            //qDebug() << "[Debug] MusicList_Group::setDataArtistFromDB " << start_index << this->draw_cnt;
    ...
    void MusicList_Group::setDataComposerFromDB(){
        ...
        if(this->total_cnt > this->jsonArr_Item.count()){
            ...
            if(err.type() == QSqlError::NoError){
                ...
                //qDebug() << "[Debug] MusicList_Group::setDataComposerFromDB " << strQuery;
                ...
                if(data.size() > 0){
                    ...
                    if(start_index == 0){
                        ...
                        //qDebug() << "[Debug] MusicList_Group::setDataComposerFromDB " << this->total_cnt << this->widget_cnt << this->draw_cnt;
                        ...
                        //qDebug() << "[Debug] MusicList_Group::setDataComposerFromDB " << start_index << max_cnt;
                        ...
                        //qDebug() << "[Debug] MusicList_Group::setDataComposerFromDB " << f_width << f_wg_cnt << f_mod << this->widget_cnt;
            ...
            //qDebug() << "[Debug] MusicList_Group::setDataComposerFromDB " << f_width << f_wg_cnt << f_mod << this->album_widget_cnt;
     ...
     */
    void MusicList_Group::setDataAddedAlbumFromDB(){
        ...
        if(this->total_cnt > this->jsonArr_Item.count()){
            ...
            if(err.type() == QSqlError::NoError){
                ...
                //qDebug() << "[Debug] MusicList_Group::setDataAddedAlbumFromDB " << strQuery;
     ...
     */
    void MusicList_Group::proc_wheelEvent_to_getMoreData(){

        //qDebug() << "[Debug] MusicList_Group::proc_wheelEvent_to_getMoreData " << this->total_cnt << this->jsonArr_Item.count() << this->draw_cnt;

        // wheel evnet change - by diskept j230317 start
        if((this->total_cnt > this->jsonArr_Item.count()) && (this->scrollArea_main->verticalScrollBar()->value() == this->scrollArea_main->verticalScrollBar()->maximum())){

            print_debug();ContentLoadingwaitingMsgShow(tr("Content is being loaded. Please wait."));

            if(groupMode==GroupMode::KIND){
                this->setDataGenreFromDB();
            }
            else if(groupMode==GroupMode::ARTIST){
                this->setDataArtistFromDB();
            }
            else if(groupMode==GroupMode::COMPOSER){
                this->setDataComposerFromDB();
            }
            else if(groupMode==GroupMode::VIEWALL_ADDEDALBUM){
                this->setDataAddedAlbumFromDB();
            }
        }
        else if((this->total_cnt > this->draw_cnt) && this->flag_item_draw == false && (this->scrollArea_main->verticalScrollBar()->value() == this->scrollArea_main->verticalScrollBar()->maximum())){
    ...
    void MusicList_Group::resizeEvent(QResizeEvent *event){//c230304

        Q_UNUSED(event);

        //qDebug() << "[Debug] MusicList_Group::resizeEvent " << this->width() << this->flowLayout_items->geometry();
...
//    vl_total->setContentsMargins(0,0,10,0);
