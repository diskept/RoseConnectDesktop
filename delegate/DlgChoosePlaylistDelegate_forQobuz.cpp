...
namespace qobuz {
    ...
    void DlgChoosePlaylistDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
        ...
        // to debug
        PlaylistItemData data = qobuz::ConvertData::convertData_playlistData(index.data(Qt::UserRole).toJsonObject());
        ...
        if(data.id <= 0){
            ...
            QString track_count = "";
            if(global.lang == 0){
                if(data.tracks_count <= 1){
                    track_count = QString("%1 Song").arg(data.tracks_count);
                }
                else{
                    track_count = QString("%1 Songs").arg(data.tracks_count);
                }
            }
            else if(global.lang == 1){
                track_count = QString("%1 곡").arg(data.tracks_count);
            }
