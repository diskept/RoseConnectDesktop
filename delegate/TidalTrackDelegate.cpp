...
namespace tidal {
    ...
    /**
     * @brief TidalTrackDelegate::TidalTrackDelegate
     * @param flagShowBtn_addCollection
     * @param parent
     */
    TidalTrackDelegate::TidalTrackDelegate(int section, UseMode useMode, QObject *parent)
     ...
     * @param index
     */
    void TidalTrackDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
        ...
        // to debug
        tidal::TrackItemData data = ConvertData::convertData_trackItemData(index.data(Qt::UserRole).toJsonObject());
        ...
        if(data.audioQuality == "HI_RES"){
            ...
            rect = option.rect.adjusted(title_width, 20, -width + title_width + 36, -37);
        ...
        painter->setPen(titlePen);
        painter->drawText(rect.left(), rect.top(), tmp_albumWidth, rect.height(), Qt::AlignLeft, ellipseString(data.albumName, tmp_albumWidth, font_16), &rect);
