 ...
 * @param parent
 */
CDFileDelegate::CDFileDelegate(QObject *parent)//c220721
    : QAbstractItemDelegate(parent)
{
    setInit();
}

void CDFileDelegate::setInit(){
 ...
 * @param index
 */
void CDFileDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
//print_debug();
    ...
    QString tmp_fileName = map["mTitle"].toString();
    QString tmp_filePath = map["filePath"].toString();
    //QString tmp_title = map["title"].toString();//c220721
    QString tmp_title = map["title"].toString();//c220721
    //qDebug() << "tmp_title=" << tmp_title;//c220721
    QString tmp_author = map["mArtist"].toString();
    //qDebug() << "tmp_author=" << tmp_author;//c220721
    QString tmp_album = map["albumTitle"].toString();
    QString tmp_duration = map["mRunningTime"].toString();
    bool tmp_flagIsFile = true;//map["isFile"].toBool();
    bool tmp_favorites = map["favorites"].toBool();//
    bool tmp_mIsPlaying = map["mIsPlaying"].toBool();
    ...
    //painter->setBackground(QBrush("#FFFFFF"));
    if(!tmp_flagIsFile){
        // 폴더명
 ...
 * @return
 */
QSize CDFileDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const{
    Q_UNUSED(option);
    Q_UNUSED(index);
    //return QSize(944,70);
 ...
 * @return
 */
QString CDFileDelegate::ellipseString(const QString &p_str, const int &p_width, const QFont &p_font) const{
 ...
 * @return
 */
bool CDFileDelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option, const QModelIndex &index)
{
    ...
    if(eventMouse->button() == Qt::LeftButton){
        ...
        QMap<QString,QVariant> map = index.data(Qt::UserRole).toMap();
        int tmp_index = map["mTrack"].toInt();
        ...
        if (event->type() == QEvent::MouseButtonPress){
            ...
            if(rectBtn_1.contains(eventMouse->pos()) && tmp_pressBtn==BtnType::playListAdd && flagIsMusicFileMode){
                ...
                //print_debug();
                qDebug() << "CDFileDelegate::editorEvent-----tmp_index=" << tmp_index;
 ...
 */
void CDFileDelegate::setVideoFileMode(){
