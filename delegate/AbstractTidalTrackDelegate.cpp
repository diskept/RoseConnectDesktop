#include "AbstractTidalTrackDelegate.h"
...
//    const int ITEM_ALL_WIDTH_MIN = 914;     ///< MIN WIDTH
    ...
    /**
     * @brief AbstractTidalTrackDelegate::AbstractTidalTrackDelegate
     * @param flagShowBtn_addCollection
     * @param parent
     */
    AbstractTidalTrackDelegate::AbstractTidalTrackDelegate(int section, UseMode useMode, QObject *parent)
        : QAbstractItemDelegate(parent)
    {
        ...
        if((useMode == UseMode_Flag::Bugs_Track_withRank) || (useMode == UseMode_Flag::Bugs_Track_General)){
            this->flagShowBtn_bugsVideo[m_section] = true;
        }
     ...
     */
    void AbstractTidalTrackDelegate::setInit(){
        ...
        this->pixmap_video.load(":images/list_video_ico.png");
        ...
        if(!this->flagShowBtn_addCollection){            
            ...
            if(!this->flagShowBtn_bugsVideo[m_section]){
                this->fixed_whole_width -= BTN_ICON_WIDTH;
            }
        }
     ...
     * @return
     */
    int AbstractTidalTrackDelegate::section(){
     ...
     * @param index
     */
    void AbstractTidalTrackDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    ...
    void AbstractTidalTrackDelegate::bugs_paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index, const bool flag) const{
        ...
        bool* p_bool = (bool*)&this->flagShowBtn_bugsVideo[index.row()];
        *p_bool = flag;

        //qDebug() << "drawing number : " << index.row() << ", value : " << this->flagShowBtn_bugsVideo[index.row()];

        // Bugs Video
        if(this->flagShowBtn_bugsVideo[index.row()] == true){
            tmp_x1 = tmp_x1 - BTN_ICON_WIDTH - (SPACE/2);
            tmp_x2 = tmp_x2 - BTN_ICON_WIDTH - (SPACE/2);
            rect = option.rect.adjusted(tmp_x1, 0, tmp_x2, 0);
            //rect = option.rect.adjusted(width-BTN_ICON_WIDTH*2-SPACE, 0, -BTN_ICON_WIDTH*1-SPACE, 0);
            painter->drawPixmap(rect, this->pixmap_video);
        }
    }
     ...
     * @return
     */
    QSize AbstractTidalTrackDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const{
        Q_UNUSED(option);
        Q_UNUSED(index);
     ...
     * @return
     */
    QString AbstractTidalTrackDelegate::ellipseString(const QString &p_str, const int &p_width, const QFont &p_font) const{
     ...
     * @return
     */
    bool AbstractTidalTrackDelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option, const QModelIndex &index) {
        ...
        if(eventMouse->button() == Qt::LeftButton){
            ...
            //qDebug() << "click number : " << index.row() << ", value : " << this->flagShowBtn_bugsVideo[index.row()];
            if(this->flagShowBtn_bugsVideo[index.row()]){
                // Bugs Video
                tmp_x1 -= (BTN_ICON_WIDTH + SPACE + (SPACE/2));
                tmp_x2 -= (BTN_ICON_WIDTH + SPACE + (SPACE/2));
                rectBtn_bugsVideo = option.rect.adjusted(tmp_x1, 0, tmp_x2, 0);
                tmp_width_allBtn += BTN_ICON_WIDTH;
            }
            ...
            if(event->type() == QEvent::MouseButtonPress){
                ...
                else if(this->flagShowBtn_bugsVideo[index.row()] && rectBtn_bugsVideo.contains((eventMouse->pos()))){
                    model->setData(index, ClickMode::MusicVideoBtn, Qt::UserRole+1);
                }
                ...
                else if(this->flagShowBtn_bugsVideo[index.row()] && tmp_pressBtn==ClickMode::MusicVideoBtn && rectBtn_bugsVideo.contains(eventMouse->pos())){
                    emit this->clicked(index.row(), ClickMode::MusicVideoBtn);
                }
