#ifndef DIALOGSETTING_OPT_H
#define DIALOGSETTING_OPT_H
 ...
 */
class dialogsetting_opt : public QDialog
{
    Q_OBJECT
public:
    explicit dialogsetting_opt(QWidget *parent = nullptr);
    QString getSelecteddeviceName() const{
        return selected_deviceName;
    };

private slots:
    void slot_clickedOptChangeSet();
  //  void slot_clickedOptsubmqaGain();
  //  void slot_clickedOptaddmqaGain();
   // void slot_clickedOptsuboutputGain();
   // void slot_clickedOptaddoutputGain();
  //  void slot_modeOnOff();
  //  void slot_print();
    void slot_changedIndexOpt(int index);
    void slot_responseHttp(const int &p_id, const QJsonObject &p_jsonObject);
    void slot_showDlgOfOptNotice();
  //  void slot_clickedOptmqaGain();
  //  void slot_clickedOptoutputGain();
...
DialogConfirm *dlgConfirmOpt;
//DialogConfirm;
    QJsonObject OptDetailData ;
...
public:
    ...
    void setDialogOpt_show();
    void setDialogOpt_save(QJsonObject opt_rcv);
  ...
  //  QWidget* getUIControlOption(QString p_title);
    ...
    QJsonObject get_settingOfOptDetail();
    void set_settingOfOptDetail(QJsonObject a);
    void getOptSettingOfsetting();
    void getOptInOutSettingOfsetting();
  //  void dlgOptPass();
    void dlgOptMSB();
    void dlgOptLSB();
 ...
 *
    ...
    QString selectedOptMQA_str;

 //   QJsonObject Opt_json = get_settingOfOptDetail();
    QJsonDocument doc0(OptDetailData);  QString strJson0(doc0.toJson(QJsonDocument::Compact));  qDebug() << "pre-Opt_json =: " << strJson0;

    switch(btnSenderNo){
    case  0 :
        selectedOptMQA_str = comboBoxOptMQA->itemText(index);
        qDebug() << " selectedOpt_str: " << selectedOptMQA_str;
        OptDetailData.insert( "mqaType", selectedOptMQA_str);
    //    set_settingOfOptDetail(OptDetailData);

        break;
    case  1 :
        selectedOptMQA_str = comboBoxOptPCM->itemText(index);
        qDebug() << " selectedOpt_str: " << selectedOptMQA_str;
        OptDetailData.insert( "samplingRate", selectedOptMQA_str);
     //   set_settingOfOptDetail(OptDetailData);


        break;
    case  2 :
        selectedOptMQA_str = comboBoxOptDSD->itemText(index);
        qDebug() << " selectedOpt_str: " << selectedOptMQA_str;
    ...
    case  3 :
        ...
        HdmiDetailData.insert( "roonVolume", selectedHdmiMQA_str);
       // set_settingOfHdmiDetail(HdmiDetailData);
        break;
    case  4 :
       // selectedHdmiMQA_str = comboBoxHdmiRoon->itemText(index);
