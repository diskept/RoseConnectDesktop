#include "dialogsetting_interoutput_150.h"
#include "dialogsetting_interoutput_250.h"
 ...
 * @param parent
 */
dialogsetting_interoutput_250::dialogsetting_interoutput_250(QWidget *parent)
    : QDialog(parent)
{
    setInit();
    setUIControl();
 ...
 */
void dialogsetting_interoutput_250::setInit(){
    this->setModal(true);
 ...
 */
void dialogsetting_interoutput_250::setUIControl(){
    ...
    lb_MQA->setFixedHeight(70);//c220511
   ...
   // comboBoxOpt_coaxDSD->addItem(tr("DOP"), 0);
   // comboBoxOpt_coaxDSD->addItem(tr("PCM"), 1);
   ...
   // this->list_keyValue.append(1);
    comboBoxOpt_coaxDSD->addItem(tr(" Native DSD"), 0);//cheon01_src
    comboBoxOpt_coaxDSD->addItem(tr(" DSD over PCM"), 1);//cheon01_src
    comboBoxOpt_coaxDSD->addItem(tr(" DSD to PCM"), 2);//cheon01_src


    //comboBox = new QComboBox();
    comboBoxOpt_coaxDSD->setContentsMargins(0,0,0,0);
    comboBoxOpt_coaxDSD->setFixedHeight(35);
    comboBoxOpt_coaxDSD->setObjectName("comboBox_opt_coax");//cheon01
...
//---------------------------------------------------------------------------------------
    ...
    lb_HOI->setFixedHeight(70);//c220511
   ...
   // this->list_keyValue.append(1);
    ...
    hl_lb_HOI->addSpacing(10);

    hl_lb_HOI->addWidget(comboBoxHOI, 0, Qt::AlignRight);//c220511

    QWidget *widget_total_HOI = new QWidget();
    widget_total_HOI->setObjectName("widget_total_DSD");
    widget_total_HOI->setStyleSheet(" border-bottom:1px solid #303030;border-left:1px solid #303030;   ");
    widget_total_HOI->setLayout(hl_lb_HOI);
    //----------------------------------------------------------------------------

    comboBoxOpt_coaxRoon->addItem(tr("OFF"), 0);
    comboBoxOpt_coaxRoon->addItem(tr("ON"), 1);
   ...
   // this->list_keyValue.append(1);
    ...
    comboBoxOpt_coaxRoon->setObjectName("comboBox_opt_coax");//cheon01
...
//---------------------------------------------------------------------------------------------------
    ...
    lb_i2s_text->setFixedSize(100,70);
    ...
    hl_lb_i2s_text->addWidget(lb_i2s);
   ...
   // wbtnDVIA->setStyleSheet("#wi2s_text {background-color:transparent;border-top:1px solid #404040; } ");
    wwbtn_DVIA->setFixedSize(340, 280);
    ...
    wwbtn_DVIB->setFixedSize(340, 280);
...
//------------------------------------------------------------------------------------------------------------------
    ...
    vl_total2->addWidget(widget_total_PCM);//cheon01

    vl_total2->addWidget(widget_total_Roon);
    vl_total2->addWidget(widget_total_DSD);//cheon01
    ...
    connect(comboBoxOutputPCMResampling, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_changedIndexOpt_coax(int)));
     //connect(comboBoxHOI, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_changedIndexOpt_coax(int)));
...
void dialogsetting_interoutput_250::slot_clickedBit32question(){//c220511
    print_debug();
    ...
    dlgConfirmOpt_coax->setTextHeight(120);
    dlgConfirmOpt_coax->setGeometry((this->geometry().left() + (DLG_WIDTH - 660) / 2), this->geometry().top() + (DLG_HEIGHT - dlgConfirmOpt_coax->sizeHint().height()) / 2, 0, 0);
...
void dialogsetting_interoutput_250::slot_clickedDSDquestion(){//c220511
    print_debug();
    ...
    dlgConfirmOpt_coax->setTextHeight(350);
    dlgConfirmOpt_coax->setGeometry((this->geometry().left() + (DLG_WIDTH - 660) / 2), this->geometry().top() + (DLG_HEIGHT - dlgConfirmOpt_coax->sizeHint().height()) / 2, 0, 0);
    dlgConfirmOpt_coax->setAlertMode();
    dlgConfirmOpt_coax->setProperty("flagShown",false);
...
void dialogsetting_interoutput_250::slot_clickedPCMquestion(){//c220511
    print_debug();
    ...
    dlgConfirmOpt_coax->setTextHeight(200);
    dlgConfirmOpt_coax->setGeometry((this->geometry().left() + (DLG_WIDTH - 660) / 2), this->geometry().top() + (DLG_HEIGHT - dlgConfirmOpt_coax->sizeHint().height()) / 2, 0, 0);
    dlgConfirmOpt_coax->setAlertMode();
    dlgConfirmOpt_coax->setProperty("flagShown",false);
...
void dialogsetting_interoutput_250::slot_clickedPreLevelquestion(){//c220511
    print_debug();
    dlgConfirmOpt_coax->setTitle(tr("PCM Resampling Frequency"));
    dlgConfirmOpt_coax->setText(tr("Fixes the pre-out output level. You can use the output level of the ROSE by holding the output level of the ROSE as an input within the acceptable range of the AMP, you connected without using the volume control of the ROSE."));

    dlgConfirmOpt_coax->setTextHeight(220);
    dlgConfirmOpt_coax->setGeometry((this->geometry().left() + (DLG_WIDTH - 660) / 2), this->geometry().top() + (DLG_HEIGHT - dlgConfirmOpt_coax->sizeHint().height()) / 2, 0, 0);
    dlgConfirmOpt_coax->setAlertMode();
    dlgConfirmOpt_coax->setProperty("flagShown",false);
...
void dialogsetting_interoutput_250::slot_clickedHOIquestion(){//c220511
    print_debug();
    dlgConfirmOpt_coax->setTitle(tr("Headphone Output Impedance"));
    dlgConfirmOpt_coax->setText(tr("This is a menu to set the Headphone Output Impedace.\nIf you select the output (16 Ohm, 32 Ohm, 50 Ohm, 100 Ohm) that fits your headphone specifications, you can hear better sound quality."));
...
void dialogsetting_interoutput_250::slot_clickedMQAquestion(){//c220511
    print_debug();
    ...
    dlgConfirmOpt_coax->setTextHeight(170);
    dlgConfirmOpt_coax->setGeometry((this->geometry().left() + (DLG_WIDTH - 660) / 2), this->geometry().top() + (DLG_HEIGHT - dlgConfirmOpt_coax->sizeHint().height()) / 2, 0, 0);
    dlgConfirmOpt_coax->setAlertMode();
    dlgConfirmOpt_coax->setProperty("flagShown",false);
...
void dialogsetting_interoutput_250::slot_clickedSoftwareVolquestion(){//c220511
    print_debug();
    ...
    dlgConfirmOpt_coax->setAlignment(Qt::AlignLeft);
    dlgConfirmOpt_coax->setTextHeight(270);
    dlgConfirmOpt_coax->setGeometry((this->geometry().left() + (DLG_WIDTH - 660) / 2), this->geometry().top() + (DLG_HEIGHT - dlgConfirmOpt_coax->sizeHint().height()) / 2, 0, 0);
...
void dialogsetting_interoutput_250::slot_clickedOpt_coaxoutputGain(){
    this->dlgOpt_coaxMqa();
    QTimer::singleShot(500, this, SLOT(slot_showDlgOfOpt_coaxNotice()));
}
void dialogsetting_interoutput_250::slot_clickedOpt_coaxmqaGain(){
    this->dlgOpt_coaxOutput();
    QTimer::singleShot(500, this, SLOT(slot_showDlgOfOpt_coaxNotice()));
}
void dialogsetting_interoutput_250::slot_clickedOpt_coaxpassGain(){
    this->dlgOpt_coaxPass();
    QTimer::singleShot(500, this, SLOT(slot_showDlgOfOpt_coaxNotice()));
}

void dialogsetting_interoutput_250::dlgOpt_coaxMqa(){
...
void dialogsetting_interoutput_250::dlgOpt_coaxOutput(){
...
void dialogsetting_interoutput_250::dlgOpt_coaxPass(){
...
void dialogsetting_interoutput_250::slot_showDlgOfOpt_coaxNotice(){
...
void dialogsetting_interoutput_250::set_settingOfOpt_coaxDetail(QJsonObject tmpJson){
        this->Opt_coaxDetailData = tmpJson;
}

QJsonObject dialogsetting_interoutput_250::get_settingOfOpt_coaxDetail(){
        return this->Opt_coaxDetailData;
}

void dialogsetting_interoutput_250::slot_clickedDVIAB(){
       ...
       // int selMode =0;
        ...
        case 77:
            ...
            ToastMsg::show(this, "", tr("PS Audio output A mode is selected."));
        ...
        case 78:
            ...
            ToastMsg::show(this, "", tr("PS Audio output B mode is selected."));
...
void dialogsetting_interoutput_250::slot_changedIndexOpt_coax(int index){
print_debug();
qDebug() << "index = " << index;
    int btnSenderNo = sender()->property("btnNo").toInt();
...
QWidget* dialogsetting_interoutput_250::getUIControlOption(QString p_title){
...
QWidget* dialogsetting_interoutput_250::getPlusMinusUIControlOption(QString p_title, int i, int data){
...
void dialogsetting_interoutput_250::slot_clickedOpt_coaxaddmqaGain(){
...
void dialogsetting_interoutput_250::slot_clickedOpt_coaxsubmqaGain(){
    if( mqadBData > -20 )
        mqadBData -= 1;
    this->lb_text[0]->setText(QString("%1").arg(this->mqadBData) + tr("           ") + tr("dB"));
    this->Opt_coaxDetailData.insert( "mqaRelplayGain", this->mqadBData);
...
void dialogsetting_interoutput_250::slot_clickedOpt_coaxaddoutputGain(){
...
void dialogsetting_interoutput_250::slot_clickedOpt_coaxsuboutputGain(){
    if( outputdBData > -20 )
        outputdBData -= 1;
 ...
 * @return
 */
QWidget* dialogsetting_interoutput_250::getUIControlOption(QString p_title, QString p_content, int index){
...
QWidget* dialogsetting_interoutput_250::getUIControlOption_32(QString p_title){
...
void dialogsetting_interoutput_250::slot_modeOnOff_32(){
...
void dialogsetting_interoutput_250::slot_modeOnOff(){
...
void dialogsetting_interoutput_250::slot_print(){
...
void dialogsetting_interoutput_250::getOpt_coaxSettingOfsetting(){
...
void dialogsetting_interoutput_250::getOpt_coaxInOutSettingOfsetting(){
...
void dialogsetting_interoutput_250::slot_clickedOpt_coaxChangeSet(){
    print_debug();
   ...
   QJsonDocument doc(tmp_json_set);  QString strJson(doc.toJson(QJsonDocument::Compact));  qDebug() <<"slot_clickedOpt_coaxChangeSet()-tmp_json_set :"<< strJson;//cheon09_io
...
void dialogsetting_interoutput_250::setDialogOpt_coax_show(){
   // QJsonObject *Opt_coax_json = get_settingOfOpt_coaxDetail();

   QJsonDocument doc(this->Opt_coaxDetailData);  QString strJson(doc.toJson(QJsonDocument::Compact));  qDebug() <<"setDialogOpt_coax_show()-Opt_coaxDetailData :"<< strJson;//cheon09_io
    ...
    comboBoxOutputPCMResampling->setCurrentIndex( Opt_coaxDetailData["samplingRate"].toInt() );
      this->comboBoxHOI->setCurrentIndex( this->Opt_coaxDetailData["hpOhm"].toInt() );
    qDebug() << "setCurrentIndex_samplingRate :" <<  this->Opt_coaxDetailData["samplingRate"].toInt();
    ...
    onOff_32->setValue(this->Opt_coaxDetailData["isClearLowerBit"].toBool());
...
void dialogsetting_interoutput_250::setDialogOpt_coax_save(QJsonObject p_jsonObject){
    QJsonObject tmp_Json;
...
void dialogsetting_interoutput_250::slot_responseHttp(const int &p_id, const QJsonObject &p_jsonObject){
...
void dialogsetting_interoutput_250::set_selected_deviceName(QString usbstr){

    this->selected_deviceName = usbstr;
}


QString dialogsetting_interoutput_250::get_selected_deviceName(){

    return this->selected_deviceName;
}


void dialogsetting_interoutput_250::setTextName(QString usbstr){

    this->radio_UsbName->setText(usbstr);
}


void dialogsetting_interoutput_250::setPropertyName(QString usbstr){

    this->radio_UsbName->setProperty(PROPERTY_NAME_READABLE, usbstr);
}

void dialogsetting_interoutput_250::loding_ani_init(){
...
void dialogsetting_interoutput_250::loding_ani_show(){
...
void dialogsetting_interoutput_250::loding_ani_hide(){
