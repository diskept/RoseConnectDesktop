#include "dialogsetting_usbsetting.h"
 ...
 * @param parent
 */
dialogsetting_usbsetting::dialogsetting_usbsetting(QWidget *parent)//c220708
    : QDialog(parent)
{
    setInit();
    setUIControl();
 ...
 */
void dialogsetting_usbsetting::setInit(){
    this->setModal(true);
...
void dialogsetting_usbsetting::setUIControl(){
    ...
    lb_title->setText(tr("USB Setting"));//lb_title->setText(tr("클라우드 추가"));
...
void dialogsetting_usbsetting::slot_clickedPassquestion(){//c220511
    print_debug();
...
void dialogsetting_usbsetting::slot_clickedbit32question(){//c220511
    print_debug();
...
void dialogsetting_usbsetting::slot_clickedHdmioutputGain(){
    dlgHdmiMqa();
    QTimer::singleShot(500, this, SLOT(slot_showDlgOfHdmiNotice()));
}


void dialogsetting_usbsetting::slot_clickedHdmimqaGain(){
    dlgHdmiOutput();
    QTimer::singleShot(500, this, SLOT(slot_showDlgOfHdmiNotice()));
}


void dialogsetting_usbsetting::slot_clickedHdmipassGain(){
    dlgHdmiPass();
    QTimer::singleShot(500, this, SLOT(slot_showDlgOfHdmiNotice()));
}


void dialogsetting_usbsetting::dlgHdmiMqa(){
...
void dialogsetting_usbsetting::dlgHdmiOutput(){
...
void dialogsetting_usbsetting::dlgHdmiPass(){
...
void dialogsetting_usbsetting::slot_showDlgOfHdmiNotice(){
...
void dialogsetting_usbsetting::set_settingOfHdmiDetail(QJsonObject tmpJson){
        HdmiDetailData = tmpJson;
}
QJsonObject dialogsetting_usbsetting::get_settingOfHdmiDetail(){
        return HdmiDetailData;
}

void dialogsetting_usbsetting::slot_changedIndexHdmi(int index){
...
QWidget* dialogsetting_usbsetting::getUIControlOption_Pass(QString p_title){
...
QWidget* dialogsetting_usbsetting::getUIControlOption_bit32(QString p_title){
...
QWidget* dialogsetting_usbsetting::getUIControlOption(QString p_title, int index){//c220511
...
QWidget* dialogsetting_usbsetting::getPlusMinusUIControlOption(QString p_title, int i, int data){
...
void dialogsetting_usbsetting::slot_clickedHdmiaddmqaGain(){
//btn_gain_add[i]
    print_debug();
    if( mqadBData < 0 )
        mqadBData += 1;
...
void dialogsetting_usbsetting::slot_clickedHdmisubmqaGain(){
    print_debug();
    if( mqadBData > -20 ){
        mqadBData -= 1;
    }
...
void dialogsetting_usbsetting::slot_clickedHdmiaddoutputGain(){
...
void dialogsetting_usbsetting::slot_clickedHdmisuboutputGain(){
    print_debug();
    if( outputdBData > -20 ){
        print_debug();
        outputdBData -= 1;
 ...
 * @return
 */
QWidget* dialogsetting_usbsetting::getUIControlOption(QString p_title, QString p_content, int index){
...
void dialogsetting_usbsetting::slot_modeOnOff_Pass(){
...
void dialogsetting_usbsetting::slot_modeOnOff_32(){
...
void dialogsetting_usbsetting::slot_print(){
...
void dialogsetting_usbsetting::getHdmiSettingOfsetting(){
...
void dialogsetting_usbsetting::getHdmiInOutSettingOfsetting(){

   NetworkHttp *network = new NetworkHttp;
   QJsonObject tmp_json;
   connect(network, SIGNAL(response(int,QJsonObject)), SLOT(slot_responseHttp(int,QJsonObject)));
   network->request(HTTP_USB_GET_MODE, QString("http://%1:%2/%3").arg(global.device.getDeviceIP()).arg(global.port).arg("in.out.mode.get"), tmp_json, true);
...
void dialogsetting_usbsetting::slot_clickedUsbChangeSet(){
   ...
   connect(network, SIGNAL(response(int,QJsonObject)), SLOT(slot_responseHttp(int,QJsonObject)));
   network->request(HTTP_USB_SET_MODE, QString("http://%1:%2/%3").arg(global.device.getDeviceIP()).arg(global.port).arg("output.setting.set"), tmp_json_set, true);

}

void dialogsetting_usbsetting::setDialogHdmi_show(){
   // QJsonObject *Hdmi_json = get_settingOfHdmiDetail();
...
void dialogsetting_usbsetting::setDialogHdmi_save(QJsonObject p_jsonObject){
    print_debug();
    QJsonObject tmp_Json;
   ...
   //  int opticalApwMode = tmp_Json["opticalApwMode"].toInt();
    // int opticalApwValue = tmp_Json["opticalApwValue"].toInt();
     ...
     HdmiDetailData.insert( "codecTrim", codecTrim);
     HdmiDetailData.insert( "dacFilterType", dacFilterType);

     HdmiDetailData.insert( "drcInfo", drcInfo);
     HdmiDetailData.insert( "dsdMode", dsdMode);
     HdmiDetailData.insert( "hpOhm", hpOhm);
     HdmiDetailData.insert( "isDigitalPassThrough", isDigitalPassThrough);
     HdmiDetailData.insert( "isHdmiPassThrough", isHdmiPassThrough);
     HdmiDetailData.insert( "javs", javs);
     HdmiDetailData.insert( "mqaRelplayGain", mqaRelplayGain);
     HdmiDetailData.insert( "mqaType", mqaType);
  //   HdmiDetailData.insert( "opticalApwMode", opticalApwMode);
    // HdmiDetailData.insert( "opticalApwValue", opticalApwValue);
     HdmiDetailData.insert( "opticalLsb", opticalLsb);
     HdmiDetailData.insert( "opticalMsb", opticalMsb);
     HdmiDetailData.insert( "outputGain", outputGain);
     HdmiDetailData.insert( "phaseInvert", phaseInvert);
     HdmiDetailData.insert( "preoutLevelInfo", preoutLevelInfo);
     HdmiDetailData.insert( "digitalVolume", digitalVolume);
     HdmiDetailData.insert( "samplingRate", samplingRate);
     HdmiDetailData.insert( "volumeMaxLevel", volumeMaxLevel);
     HdmiDetailData.insert( "volumeMinLevel", volumeMinLevel);

     qDebug() << "json-isHdmiPassThrough : " << HdmiDetailData["isHdmiPassThrough"].toBool();
...
void dialogsetting_usbsetting::slot_responseHttp(const int &p_id, const QJsonObject &p_jsonObject){
...
void dialogsetting_usbsetting::set_selected_deviceName(QString usbstr){
    selected_deviceName = usbstr;
}
QString dialogsetting_usbsetting::get_selected_deviceName(){
    return selected_deviceName;
}
void dialogsetting_usbsetting::setTextName(QString usbstr){
