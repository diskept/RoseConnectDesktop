    ...
    m_socket(socket)
{
    ...
    this->linker = Linker::getInstance();//cheon210617-http
...
void httpServerConnection::setRequestBody(QJsonObject p_jsonObject){//c211213l
    ...
    qDebug() << "httpServerConnection::setRequestBody---------------global.window_activate_flag----" << global.window_activate_flag;
    ...
    if(p_jsonObject.contains("dataObj")){
        ...
        if(p_jsonObject.contains("messageType")){
            ...
            if(p_jsonObject["messageType"].toString() == "show_toast_noti"){//c221116_1
                print_debug();
                qDebug() << "message = " << tmp_Json["message"].toString();
                //ToastMsg::show(this, "", tmp_Json["message"].toString());

                if(tmp_Json["message"].toString() == "Connection reset by peer"){//c230326
                    ToastMsg::show(this,"", tmp_Json["message"].toString(), 2000, -60, -1);
                    emit linker->signal_connect_reset_notice();//c230326
                }
                if(tmp_Json["message"].toString() == "BUGS App의 계정 상태를 확인해주세요."){//c230326
                    print_debug();
                ...
                if(global.show_flag == false){
                    ...
                    if("The volume can be adjusted when the pre-out level is turned off."== tmp_Json["message"].toString()){
                        ...
                        if(tmp_Json["message"].toString() == "The cache is locked." || tmp_Json["message"].toString() == "The cache has been unlocked."){
                            emit this->linker->signal_cacheState_change();
                        }
                        else if(tmp_Json["message"].toString() == "The cache file has been deleted."){
                            emit this->linker->signal_cacheList_change();
                        }

                        if(global.window_activate_flag == true){
                            ToastMsg::show(this,"", tmp_Json["message"].toString(), 2000, -60, -1);
                        }
                    }
                }
                    ...
                    if("The volume can be adjusted when the pre-out level is turned off."== tmp_Json["message"].toString()){
                        ToastMsg::show(this,"", tmp_Json["message"].toString(), 2000, 0, -1);//
                    }else{
                        if(tmp_Json["message"].toString() == "The cache is locked." || tmp_Json["message"].toString() == "The cache has been unlocked."){
                            emit this->linker->signal_cacheState_change();
                        }
        ...
        if(global.device.CurrPlayType =="CD"){//c220711
            ...
            //emit this->linker->signal_httpservermsg(p_jsonObject);
        }
    }
    else{//cheon211203
        ...
        if(p_jsonObject.contains("messageType")){
            ...
            if(messageType == "music_start"  ){////c220716
                ...
                if(global.queueTimerFlag && global.device.CurrPlayType != RADIO && global.device.CurrPlayType != ROSE_RADIO){
                    ...
                    emit this->linker->signal_queuelist_musicStart();//c220704
            ...
            if(global.Queue_track_count > 0 ){
                ...
                if( messageType == "queue_play" && global.device.CurrPlayType != RADIO && global.device.CurrPlayType != ROSE_RADIO){//c220826_2
                    print_debug();
                    ...
                    if(global.queueTimerFlag){
                        ...
                        //emit this->linker->signal_queuelist_musicStart();//c230429
                    }
                }
            }
            if( messageType == "queue_recent_track_add" && global.device.CurrPlayType != RADIO && global.device.CurrPlayType != ROSE_RADIO){
                print_debug();
                if(global.queueTimerFlag){
                    //emit linker->signal_queue_recent_track_add();//c230429
                    emit this->linker->signal_queuelist_musicStart();//c220826_2
                    global.queue_recent_track_addFlag = true;
                }else{
                    global.queue_recent_track_addFlag = true;//c230429
                }
                if(global.show_flag == false){//c221001_1
                    //print_debug();
                    global.show_flag =  true;
                ...
                print_debug();
                if(global.queueTimerFlag){
                    emit this->linker->signal_recommand_data_change();//c220826_2
                    //emit this->linker->signal_queuelist_reload();//cheon211206
                    //global.queue_position_startFlag = false;
                }else{
                    emit this->linker->signal_recommand_data_change();//c230429
                }
                if(global.show_flag == false){//c221001_1
                    //print_debug();
                    global.show_flag =  true;
                ...
                if(global.queueTimerFlag){
                    //emit this->linker->signal_queue_recent_track_add();//c220418
                    //global.queue_recent_track_addFlag = true;
                    emit this->linker->signal_queuelist_musicStart();//c220411
                    global.queue_music_startFlag = true;
                }
            }
            ...
            //else if( messageType == "queue_play"  && global.device.CurrPlayType != RADIO && global.device.CurrPlayType != ROSE_RADIO){//c220826_2
                print_debug();
                //global.queue_recent_track_addFlag = true;
                /*//c220716
                ...
                qDebug() << "global.desktopPlayerMode=" << global.desktopPlayerMode;
                if(global.queueTimerFlag){
                     emit this->linker->signal_queuelist_musicStart();//c220627
                }
                if(global.desktopPlayerMode){
                    emit this->linker->signal_deviceStart();//c220609
                }
                */
            ...
            else if(messageType == "play_state_change"){
                print_debug();
                if(!global.queueTimerFlag){//c230330
                    print_debug();
                    global.Queue_track_count = 0;
                }
                emit this->linker->signal_queuelist_musicStart();//c230430
                //if(global.queueTimerFlag){//c220521
                //emit this->linker->signal_checkQueue(23, "Play state changed");//c221001_1
                if(global.show_flag == false){//c221001_1
                    //print_debug();
                    global.show_flag =  true;
                ...
                this->linker->signal_reload_setting_clicked("inputOutput");//c221020_1
                //emit this->linker->signal_device_connect_check_error();
                //emit this->linker->signal_queuelist_reload();//cheon211206
                //global.queue_position_startFlag = true;
                //}
            }
            else if(messageType == "state_check"){
                print_debug();
                //if(global.queueTimerFlag){//c220521
                emit this->linker->signal_checkQueue(28, "");//c220624
                //emit this->linker->signal_checknetwork();//c220715
                //emit this->linker->signal_device_connect_check_error();//c220526
                //emit this->linker->signal_queuelist_reload();//cheon211206
                //global.queue_position_startFlag = true;
                //}
            }
            else if(messageType == "music_play_type_change"){//c220407
                print_debug();
                //if(global.queueTimerFlag){

                //emit this->linker->signal_httpservermsg_music_play_type_change();//c220407
                //emit this->linker->signal_queuelist_reload();//cheon211206
                //global.queue_position_startFlag = true;
                //}
                emit this->linker->signal_httpservermsg(p_jsonObject);//-------------
            }
            else if(messageType == "login_state_change" || messageType == "user_login_noti" || messageType == "user_logout_noti"){
                print_debug();
                emit this->linker->signal_httpservermsg(p_jsonObject);//--------
            ...
            else if(messageType == "volume_change"){
                int volume = p_jsonObject.value("volume").toInt();

                emit this->linker->signal_volume_change(volume);
             }
            else if(messageType == "mute_state_change_noti"){
                int mute_state = p_jsonObject.value("position").toInt();        ///< Mute enable : 1, disable : 0

                emit this->linker->signal_volume_muteState(mute_state);
            }
        }
    }
}
