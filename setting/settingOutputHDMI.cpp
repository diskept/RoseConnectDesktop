...
const int HTTP_HDMI_GET_MODE  = 0;
const int HTTP_HDMI_SET_MODE  = 1;


SettingOutputHDMI::SettingOutputHDMI(QWidget *parent)//c220708
    : QDialog(parent)
{
    this->setInit();
    this->setUIControl();
...
void SettingOutputHDMI::setInit(){

    this->deviceType = global.device.getDeviceType();
    this->jsonObj_HDMI_Setting = QJsonObject();
...
void SettingOutputHDMI::setUIControl(){

    this->config_hdmi = new QWidget();
    this->config_hdmi->setStyleSheet("background-color:#333333; border:1px solid #4D4D4D;");
    this->config_hdmi->setFixedSize(DLG_WIDTH, DLG_HEIGHT);
    this->config_hdmi->hide();

    QWidget *hdmi_title = new QWidget(this->config_hdmi);
    hdmi_title->setStyleSheet("background-color:transparent; border:1px solid #4D4D4D;");
    hdmi_title->setFixedSize(DLG_WIDTH, WIDGET_HEIGHT);
    hdmi_title->setGeometry(0, 0, 0, 0);

    QLabel *label_title = new QLabel(hdmi_title);
    ...
    label_title->setText("HDMI " + tr("Setting"));
    label_title->setAlignment(Qt::AlignCenter);
    label_title->setFixedSize(DLG_WIDTH, 70);
    label_title->setGeometry(0, 0, 0, 0);

    QPushButton *btn_close = new QPushButton(hdmi_title);
    ...
    QWidget *pcm_freq = new QWidget(this->config_hdmi);
    ...
    if(this->deviceType == "RS130"){
        this->comboBox_dsdMode->addItem(tr("  Native DSD(Not supported)"), 0);
    ...
    QWidget *software_volume = new QWidget(this->config_hdmi);
    ...
    software_volume->setGeometry(0, 287, 0, 0);
    ...
    QWidget *hdmi_passthrough = new QWidget(this->config_hdmi);
    hdmi_passthrough->setStyleSheet("background-color:transparent; border:1px solid #4D4D4D;");
    hdmi_passthrough->setFixedSize(DLG_WIDTH, WIDGET_HEIGHT);
    hdmi_passthrough->setGeometry(0, 356, 0, 0);

    QLabel *label_hdmiPass = new QLabel(hdmi_passthrough);
    label_hdmiPass->setStyleSheet("background-color:transparent; border:none; font-size:20px; font-weight:normal; font-style:normal; line-height:0.65; text-align:center; color:#CCCCCC;");
    label_hdmiPass->setText("HDMI Passthrough");
    label_hdmiPass->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
    label_hdmiPass->setFixedSize(DLG_WIDTH - 35, 70);
    label_hdmiPass->setGeometry(35, 0, 0, 0);

    QPushButton *btn_hdmiPass = new QPushButton(hdmi_passthrough);
    btn_hdmiPass->setProperty("type", "passThrough");
    btn_hdmiPass->setStyleSheet("background-color:transparent; border:none;");
    btn_hdmiPass->setCursor(Qt::PointingHandCursor);
    btn_hdmiPass->setFixedSize(30, 30);
    btn_hdmiPass->setGeometry(215, 20, 0, 0);

    QLabel *img_hdmiPass = new QLabel(btn_hdmiPass);
    img_hdmiPass->setStyleSheet("background-color:transparent; background-image: url(:/images/setting/201/tip_icon.png); border:none;");
    img_hdmiPass->setFixedSize(30, 30);
    img_hdmiPass->setGeometry(0, 0, 0, 0);

    this->onoff_pass = new OnOffWidget(hdmi_passthrough);
    this->onoff_pass->setStyleSheet("border:none;");
    this->onoff_pass->setCursor(Qt::PointingHandCursor);
    this->onoff_pass->setFixedSize(74, 34);
    this->onoff_pass->setGeometry(697, 19, 0, 0);

    QWidget *DACs32bit = new QWidget(this->config_hdmi);
    ...
    this->btn_apply = new QPushButton(this->config_hdmi);
    ...
    this->btn_cancel = new QPushButton(this->config_hdmi);
    ...
    connect(btn_hdmiPass, SIGNAL(clicked()), this, SLOT(slot_clickedQuestion_clarification()));
    ...
    QVBoxLayout *vBox_config_hdmi = new QVBoxLayout();
    vBox_config_hdmi->setContentsMargins(0, 0, 0, 0);
    vBox_config_hdmi->setAlignment(Qt::AlignTop);

    GSCommon::clearLayout(vBox_config_hdmi);
    vBox_config_hdmi->setAlignment(Qt::AlignTop);
    vBox_config_hdmi->addWidget(this->config_hdmi);

    this->setLayout(vBox_config_hdmi);
}


void SettingOutputHDMI::getOutputSettingHDMI(){

    int tmpValue = 0;
    bool tmpbool = false;

    if(this->deviceType == "RS130" || this->deviceType == "RS520"){
        if(this->jsonObj_HDMI_Setting.contains("maxSamplingRate")){
            tmpValue = ProcJsonEasy::getInt(this->jsonObj_HDMI_Setting, "maxSamplingRate");
            this->comboBox_maxSampleRate->setCurrentIndex(tmpValue);
        }

        if(this->jsonObj_HDMI_Setting.contains("samplingRate")){
            tmpValue = ProcJsonEasy::getInt(this->jsonObj_HDMI_Setting, "samplingRate");
            this->comboBox_sampleRate->setCurrentIndex(tmpValue);
        }

        if(this->jsonObj_HDMI_Setting.contains("mqaResampling")){
            tmpValue = ProcJsonEasy::getInt(this->jsonObj_HDMI_Setting, "mqaResampling");
            this->checkbox_mqaStream->setChecked(tmpValue == 1 ? true : false);
        }

        if(this->jsonObj_HDMI_Setting.contains("dsdMode")){
            tmpValue = ProcJsonEasy::getInt(this->jsonObj_HDMI_Setting, "dsdMode");
            this->comboBox_dsdMode->setCurrentIndex(tmpValue);
        }

        if(this->jsonObj_HDMI_Setting.contains("digitalVolume")){
            tmpValue = ProcJsonEasy::getInt(this->jsonObj_HDMI_Setting, "digitalVolume");
            this->onoff_svc->setValue(tmpValue == 1 ? true : false);
        }

        if(this->jsonObj_HDMI_Setting.contains("isHdmiPassThrough")){
            tmpbool = ProcJsonEasy::getBool(this->jsonObj_HDMI_Setting, "isHdmiPassThrough");
            this->onoff_pass->setValue(tmpbool);
        }

        if(this->jsonObj_HDMI_Setting.contains("isClearLowerBit")){
            tmpbool = ProcJsonEasy::getBool(this->jsonObj_HDMI_Setting, "isClearLowerBit");
            this->onoff_32bit->setValue(tmpbool);
        }
    }

    if(this->config_hdmi->isVisible() == false){
        this->config_hdmi->show();
    }
}


void SettingOutputHDMI::setOutputSettingHDMI(){
    ...
    if(this->deviceType == "RS130" || this->deviceType == "RS520"){
        this->jsonObj_HDMI_Setting.remove("code");
        this->jsonObj_HDMI_Setting.remove("status");
        this->jsonObj_HDMI_Setting.remove("version");

        tmpInt = ProcJsonEasy::getInt(this->jsonObj_HDMI_Setting, "maxSamplingRate");
        tmpCompareInt = this->comboBox_maxSampleRate->currentIndex();

        if(tmpInt != tmpCompareInt){
            this->jsonObj_HDMI_Setting.remove("maxSamplingRate");
            this->jsonObj_HDMI_Setting.insert("maxSamplingRate", tmpCompareInt);
        }

        tmpInt = ProcJsonEasy::getInt(this->jsonObj_HDMI_Setting, "samplingRate");
        tmpCompareInt = this->comboBox_sampleRate->currentIndex();

        if(tmpInt != tmpCompareInt){
            this->jsonObj_HDMI_Setting.remove("samplingRate");
            this->jsonObj_HDMI_Setting.insert("samplingRate", tmpCompareInt);
        }

        tmpInt = ProcJsonEasy::getInt(this->jsonObj_HDMI_Setting, "mqaResampling");
        tmpCompareInt = this->checkbox_mqaStream->isChecked() == true ? 1 : 0;

        if(tmpInt != tmpCompareInt){
            this->jsonObj_HDMI_Setting.remove("mqaResampling");
            this->jsonObj_HDMI_Setting.insert("mqaResampling", tmpCompareInt);
        }

        tmpInt = ProcJsonEasy::getInt(this->jsonObj_HDMI_Setting, "dsdMode");
        tmpCompareInt = this->comboBox_dsdMode->currentIndex();

        if(tmpInt != tmpCompareInt){
            this->jsonObj_HDMI_Setting.remove("dsdMode");
            this->jsonObj_HDMI_Setting.insert("dsdMode", tmpCompareInt);
        }

        tmpInt = ProcJsonEasy::getInt(this->jsonObj_HDMI_Setting, "digitalVolume");
        tmpCompareInt = this->onoff_svc->getCurrentValue() == true ? 1 : 0;

        if(tmpInt != tmpCompareInt){
            this->jsonObj_HDMI_Setting.remove("digitalVolume");
            this->jsonObj_HDMI_Setting.insert("digitalVolume", tmpCompareInt);
        }

        tmpBool = ProcJsonEasy::getBool(this->jsonObj_HDMI_Setting, "isHdmiPassThrough");
        tmpConpareBool = this->onoff_pass->getCurrentValue();

        if(tmpBool != tmpConpareBool){
            this->jsonObj_HDMI_Setting.remove("isHdmiPassThrough");
            this->jsonObj_HDMI_Setting.insert("isHdmiPassThrough", tmpConpareBool);
        }
        ...
        if(tmpBool != tmpConpareBool){
            this->jsonObj_HDMI_Setting.remove("isClearLowerBit");
            this->jsonObj_HDMI_Setting.insert("isClearLowerBit", tmpConpareBool);
        }

        tmpJson.insert("data", this->jsonObj_HDMI_Setting);
        tmpJson.insert("outputMode", 2);
    }
...
//    qDebug() << "[Debug]" << __FUNCTION__ << __LINE__ << strJson;
    ...
    NetworkHttp *network = new NetworkHttp;
    connect(network, SIGNAL(response(int,QJsonObject)), SLOT(slot_responseHttp(int,QJsonObject)));
    network->request(HTTP_HDMI_SET_MODE, QString("http://%1:%2/%3").arg(global.device.getDeviceIP()).arg(global.port).arg("output.setting.set"), tmpJson, true);
}


void SettingOutputHDMI::slot_responseHttp(const int &p_id, const QJsonObject &p_jsonObject){
...
//    qDebug() << "[Debug]" << __FUNCTION__ << __LINE__ << "RCV ID :" << p_id << strJson;
    ...
    switch(p_id){
            ...
            if(p_jsonObject.contains(jsonKey_flagOk) && p_jsonObject[jsonKey_flagOk].toBool()){
                if(this->deviceType == "RS130" || this->deviceType == "RS520"){
                    if(p_jsonObject.contains("data")){
                        this->jsonObj_HDMI_Setting = ProcJsonEasy::getJsonObject(p_jsonObject, "data");
                        this->getOutputSettingHDMI();

                        emit this->signal_loading_stateHDMI(false);
                    }
                }
            }
            break;
        ...
        case HTTP_HDMI_SET_MODE:
            if(p_jsonObject.contains(jsonKey_flagOk) && p_jsonObject[jsonKey_flagOk].toBool()){
                if(this->deviceType == "RS130" || this->deviceType == "RS520"){
                    emit this->signal_setOutput_stateHDMI(p_jsonObject);
                    emit this->signal_loading_stateHDMI(false);
...
void SettingOutputHDMI::slot_clickedQuestion_clarification(){
    ...
    if(type == "pcmFreq"){
        ...
        notice->setTitle("HDMI Passthrough");
        notice->setTextHeight(60);
        notice->setAlignmentContent(Qt::AlignLeft);
        notice->setStyleSheetContent("background-color:transparent; border:none; font-size:18px; font-weight:normal; font-style:normal; line-height:2.52; text-align:center; color:#FFFFFF;");
        notice->setText(tr("It bypasses HD audio formats such as surround sound sources to the audio reciever though HDMI without decording."));
...
void SettingOutputHDMI::slot_clickedCheckbox(){
...
void SettingOutputHDMI::slot_clickedApply(){

    emit this->signal_loading_stateHDMI(true);

    this->setOutputSettingHDMI();
}
