 ...
 * @param parent
 */
FriendProfileItemRoseHome::FriendProfileItemRoseHome(ProfileBtnType p_type, QWidget *parent) : QWidget(parent)
{
...
void FriendProfileItemRoseHome::setUIControl(){
    ...
    */
    ...
    case FriendProfileItemRoseHome::ProfileBtnType::FriendList :
        hl_total->addWidget(btn_unconnect);
        break;
    case FriendProfileItemRoseHome::ProfileBtnType::Receive :
        hl_total->addWidget(btn_reject);
        hl_total->addSpacing(22);
        hl_total->addWidget(btn_accept);
        break;
    case FriendProfileItemRoseHome::ProfileBtnType::Send :
        hl_total->addWidget(btn_cancel);
        break;
    case FriendProfileItemRoseHome::ProfileBtnType::Find :
    ...
    // 커넥션
    connect(btn_unconnect, &QPushButton::clicked, this, &FriendProfileItemRoseHome::slot_clickedUnConnect);
    connect(btn_reject, &QPushButton::clicked, this, &FriendProfileItemRoseHome::slot_clickedReject);
    connect(btn_accept, &QPushButton::clicked, this, &FriendProfileItemRoseHome::slot_clickedAccept);
    connect(btn_cancel, &QPushButton::clicked, this, &FriendProfileItemRoseHome::slot_clickedCancel);
    connect(btn_request, &QPushButton::clicked, this, &FriendProfileItemRoseHome::slot_clickedRequest);
}
 ...
 * @param p_jsonObject
 */
void FriendProfileItemRoseHome::setDataProfileItem(QJsonObject p_jsonObject){//cheon211018
 ...
 */
void FriendProfileItemRoseHome::slot_loadImageMainProfile(){
 ...
 */
void FriendProfileItemRoseHome::slot_clickedUnConnect(){//cheon211023
print_debug();
qDebug() << "this->lb_userid->text():" << this->lb_userid->text();
/*
 ...
 */
void FriendProfileItemRoseHome::slot_clickedReject(){//cheon211018
print_debug();
/*
    NetworkHttp *network = new NetworkHttp();
    connect(network, SIGNAL(response(int,QJsonObject)), SLOT(slot_responseHttp(int,QJsonObject)));
    ...
    //params.addQueryItem("friendname",this->lb_username->text());
    network->request(HTTP_GET_FPI_REJECT, QString("%1/member/friend/deny").arg(global.legacy_v1), json, true, true);

}
/**
 * @brief FriendProfileItem::slot_clickedAccept [SLOT] 수락 버튼 클릭시
 */
void FriendProfileItemRoseHome::slot_clickedAccept(){//cheon211018
print_debug();
/*
    NetworkHttp *network = new NetworkHttp();
    connect(network, SIGNAL(response(int,QJsonObject)), SLOT(slot_responseHttp(int,QJsonObject)));
 ...
 */
void FriendProfileItemRoseHome::slot_clickedCancel(){//cheon211023
print_debug();
/*
    NetworkHttp *network = new NetworkHttp();
    connect(network, SIGNAL(response(int,QJsonObject)), SLOT(slot_responseHttp(int,QJsonObject)));
 ...
 */
void FriendProfileItemRoseHome::slot_clickedRequest(){//cheon211018//cheon211018
print_debug();
/*
    NetworkHttp *network = new NetworkHttp();
    connect(network, SIGNAL(response(int,QJsonObject)), SLOT(slot_responseHttp(int,QJsonObject)));
 ...
 */
void FriendProfileItemRoseHome::slot_responseHttp(const int &p_id, const QJsonObject &p_jsonObject){//cheon211023
