...
namespace tidal {
    ...
    /**
     * @brief Dialog_ChoosePlaylist_forTidal::Dialog_ChoosePlaylist_forTidal
     * @param parent
     */
    Dialog_ChoosePlaylist_forTidal::Dialog_ChoosePlaylist_forTidal(QWidget *parent) : QDialog(parent) {
        // data
...
//        tidal::PlaylistItemData tmp_new;
...
//        this->add_listWidgetItem(0);
     ...
     * @brief 다이얼로그 관련 초기설정
     */
    void Dialog_ChoosePlaylist_forTidal::setInit(){
        this->setModal(true);
...
//        this->setAttribute(Qt::WA_TranslucentBackground);
     ...
     */
    void Dialog_ChoosePlaylist_forTidal::setUIControl(){
     ...
     */
    void Dialog_ChoosePlaylist_forTidal::setUIControl_topTitle(){
        ...
        connect(btn_close, &QPushButton::clicked, this, &Dialog_ChoosePlaylist_forTidal::reject);
     ...
     */
    void Dialog_ChoosePlaylist_forTidal::setUIControl_body(){

        this->delegate = new DlgChoosePlaylistDelegate(this);
        connect(delegate, &DlgChoosePlaylistDelegate::clicked, this, &Dialog_ChoosePlaylist_forTidal::slot_clickedItem_inList);
       ...
       tidal::PlaylistItemData tmp_list_playlist;
    ...
    /**
     * @brief Dialog_ChoosePlaylist_forTidal::showEvent
     */
    void Dialog_ChoosePlaylist_forTidal::showEvent(QShowEvent *){

        ProcCommon *proc = new ProcCommon(this);
        connect(proc, &ProcCommon::completeReq_list_myCreatedPlaylists, this, &Dialog_ChoosePlaylist_forTidal::slot_applyResult_myCreatedPlaylists_forAddCollection);
        proc->request_tidal_getList_myCreatedPlaylists(ProcCommon::DataOrderOption::DATE, ProcCommon::DataOrderDirection::DESC, 50, 0);
    }


    /**
     * @brief Dialog_ChoosePlaylist_forTidal::slot_clickedItem_inList
     * @param index
     */
    void Dialog_ChoosePlaylist_forTidal::slot_clickedItem_inList(const int index){

        /*if(this->list_playlist->at(index).uuid.isEmpty()){
            // 신규 플레이리스트 추가 필요함
        }
        else{
            ...
            if(this->m_content_type == ItemType_forAddCollection::Track){
                // Track 에 대해서 담기 진행
                //qDebug() << "[TIDAL][MSG][ITEM] list_playlist.uuid : " << this->list_playlist->at(index).uuid;
                emit this->signal_choosePlaylist_forTrack(this->list_playlist->at(index).uuid, this->m_content_id.toInt());
            }
            else if(this->m_content_type == ItemType_forAddCollection::Album){
                // Album 에 대해서 담기 진행
                emit this->signal_choosePlaylist_forAlbum(this->list_playlist->at(index).uuid, this->m_content_id.toInt());
            }
            else if(this->m_content_type == ItemType_forAddCollection::Playlist){
                // Playlist 에 대해서 담기 진행
                emit this->signal_choosePlaylist_forPlaylist(this->list_playlist->at(index).uuid, this->m_content_id.toString());
            }
        ...
        if(index == 0){
            ...
            if(tmp_result == QDialog::Accepted){
                ...
                emit this->signal_choosePlaylist_forTIDAL(0, dlg_newPlaylist->getTitle(), dlg_newPlaylist->getDestription());
            ...
            emit this->signal_choosePlaylist_forTIDAL(this->list_playlist->at(index).uuid, this->list_playlist->at(index).title, this->list_playlist->at(index).description);
     ...
     */
    void Dialog_ChoosePlaylist_forTidal::slot_applyResult_myCreatedPlaylists_forAddCollection(const QList<tidal::PlaylistItemData>& p_list_playlist, const QJsonArray&, const bool, const QJsonObject&){
    ...
    /**
     * @brief Dialog_ChoosePlaylist_forTidal::slot_completeItems
     */
    void Dialog_ChoosePlaylist_forTidal::slot_completeItems(){
    ...
    /**
     * @brief Dialog_ChoosePlaylist_forTidal::add_listWidgetItem
     * @param start_index
     */
    void Dialog_ChoosePlaylist_forTidal::add_listWidgetItem(int start_index){
        ...
        for(int i = start_index; i < this->list_playlist->count(); i++){
            ...
            if(tmp_imgPath.isEmpty()){
                if(this->list_playlist->at(i).uuid.isEmpty()){
                    tmp_item->setData(Qt::DecorationRole, pixmap_new);
                }
                else{
                ...
                connect(fileDownloader, &FileDownloader::downloaded, this, &Dialog_ChoosePlaylist_forTidal::slot_thumbnailDownloaded_fotListWidget_playlist);
    ...
    /**
    * @brief Dialog_ChoosePlaylist_forTidal::set_contentData
    * @param content_id
    * @param content_type
    */
    void Dialog_ChoosePlaylist_forTidal::set_contentData(QVariant content_id, ItemType_forAddCollection content_type){
     ...
     */
    void Dialog_ChoosePlaylist_forTidal::slot_thumbnailDownloaded_fotListWidget_playlist(){
