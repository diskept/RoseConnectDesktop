...
/**
 * @brief PodcastCategory::PodcastCategory : 팟캐스트-카테고리
...
/**
 * @brief PodcastCategory::setInit : 초기 세팅
 */
void PodcastCategory::setInit(){

}

/**
 * @brief PodcastCategory::setUIControl : UI 세팅
 */
void PodcastCategory::setUIControl(){
...
/**
 * @brief PodcastCategory::setResultOfCategory : 카테고리 목록 요청결과 처리
 * @param p_jsonData QJsonObject response
 */
void PodcastCategory::setResultOfCategory(const QJsonObject &p_jsonData){
                ...
                if(p_jsonData.contains(jsonKey_data)){
                    ...
                    for(int i = 0; i < jsonArrData.size(); i++){
                        ...
                        if(jsonCategory.contains(jsonKey_genreID) && jsonCategory.contains(jsonKey_genreName)){
                            appendCategoryUI(jsonCategory[jsonKey_genreID].toInt(), jsonCategory[jsonKey_genreName].toString());
                        }
                    }
                }
            }
...
/**
 * @brief PodcastCategory::appendCategoryUI : 카테고리 UI 추가
 * @param p_id int 카테고리 아이디
 * @param p_name QString 카테고리명
 */
void PodcastCategory::appendCategoryUI(const int &p_id, const QString &p_name){
...
/**
 * @brief PodcastCategory::requestData : 카테고리 데이터 요청
 */
void PodcastCategory::requestData(){
...
/**
 * @brief PodcastCategory::slot_responseHttp : [슬롯] Http 요청 결과
 * @param p_id int 요청 아이디
 * @param p_jsonObject QJsonObject response
 */
void PodcastCategory::slot_responseHttp(const int &p_id, const QJsonObject &p_jsonObject){
 ...
 */
void PodcastCategory::slot_clickedItem(){
